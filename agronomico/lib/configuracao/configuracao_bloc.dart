...
import 'package:agronomico/base/base_event.dart';
...
class ConfiguracaoBloc extends Bloc<ConfiguracaoEvent, ConfiguracaoState> {
  ...
  Stream<ConfiguracaoState> mapEventToState(ConfiguracaoEvent event) async* {
    if (event is Iniciar) {
      String chave = await preferenciaRepository.get(idPreferencia: 'chave') ?? 'prod';
    ...
    if (event is EnviarEmail) {
      ...
      try {
        ...
        String path = join(databasesPath, kNomeDb);
        File db = File(path);
